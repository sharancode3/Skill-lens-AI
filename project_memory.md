...
- [x] Phase 12 - Part B: Proctoring Violation System Reliability Overhaul (implemented a 500ms debounced `registerPotentialExit()` client event pipeline coalescing raw focus/visibility signals, wired 10-second warning lockout overlay countdowns, mapped atomic server-confirmed counters, created `/api/session/:sessionId` for recovery on page reload, and automated validation via `testProctoringDebounce.js`).
- [x] Phase 12 - Part C: MCQ Dead-End Hardening (built a server-side hard validation in `handleTurn()` that automatically re-tags MCQ to free-text if choices are missing/sparse, added frontend check in `app.js` to ensure input fallback when choices are < 2, and created `testMCQDeadEndFallback.js` regression unit test).
- [x] Phase 12 - Part D: Natural Interviewer Spoken Phrasing & QA Verification (updated system prompts in `llmClient.js` to enforce self-contained scenario questions without leaking topic day number prefixes, structured plausible MCQ distractors, integrated a self-check understandability LLM query with a single-pass regeneration loop, and rewrote offline mock stems to match spoken technical formats).
- [x] Phase 12 - Part E: Stopping Condition & Queue Size Guarantees (implemented single-source-of-truth stopping condition gate `checkStoppingCondition()` evaluated at the top and bottom of each turn, expanded minimum topic queue size to at least 8 topics, updated `isFinalQuestion` signal path to support capstone trigger logic, and automated checks in `testStoppingConditionPartE.js`).
